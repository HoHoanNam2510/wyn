...
import type { GrammarQuizQuestion } from '@/lib/grammarQuiz/pickQuestions';
import {
  FormulaDisplay,
  type FormulaChunk,
} from '@/components/grammar/formula-display';
import { logGrammarReviewEvent } from '@/app/actions/grammarQuiz';
...
export function GrammarQuizSessionClient({
  questions,
}: {
  ...
  const handleChoice = useCallback(
    (choiceId: string) => {
      ...
      const q = currentQ;
      const correct = choiceId === q.correctPatternId;
      ...
      setAnswers((prev) => [
        ...
        {
          sentence: q.sentence,
          userChoiceTitle: userChoice?.title ?? '',
          correctPatternTitle: q.correctPatternTitle,
          correctPatternFormula: q.correctPatternFormula,
          correct,
        },
      ]);

      logGrammarReviewEvent({
        patternId: q.correctPatternId,
        correct,
        durationMs,
      }).catch(() => {});
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1 min-w-0">
                      <p className="text-sm font-semibold text-primary">
                        {a.correctPatternTitle}
                      </p>
                      <FormulaDisplay chunks={a.correctPatternFormula} />
                    </div>
                  </div>
                </div>
              ))}
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm font-medium text-muted-foreground text-center">
        Which grammar pattern does this sentence demonstrate?
      </p>

      {/* Choice grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {currentQ.choices.map((choice, i) => {
          const isCorrect = choice.id === currentQ.correctPatternId;
